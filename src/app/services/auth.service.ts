...
export class AuthService {
  ...
  SignUp(email, password, name, studnetID, ncod) {
    return this.afAuth.createUserWithEmailAndPassword(email, password)
      .then((result) => {
        /* Call the SendVerificaitonMail() function when new user sign 
        up and returns promise */
        this.SendVerificationMail();
        this.SetUserDataForSignUP(result.user,email, name, studnetID, ncod);
        this.router.navigate(['studMain']);
      }).catch((error) => {
        window.alert(error.message)
      })
  ...
  SetUserDataForSignUP(user, email, name, studnetID, ncod) {
    const userRef: AngularFirestoreDocument<any> = this.afs.doc(`Student/${user.uid}`);
    const userData: Student = {
      studentID: studnetID,
      uid: user.uid,
